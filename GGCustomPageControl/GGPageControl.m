...
-(void)exchangeCurrentView:(NSInteger)old new:(NSInteger)new
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        CGFloat lx=mpSelect.origin.x;
         if(new<old)
             lx+=(self->_currentControlSize.width - self->_otherControlSize.width);
        oldSelect.frame=CGRectMake(lx, mpSelect.origin.y, self->_otherControlSize.width, self->_otherControlSize.height);
 
        CGFloat mx=newTemp.origin.x;
        if(new>old)
            mx-=(self->_currentControlSize.width - self->_otherControlSize.width);
        newSeltect.frame=CGRectMake(mx, newTemp.origin.y, self->_currentControlSize.width, self->_currentControlSize.height);
        ...
        if(new-old>1)
        {
            for(NSInteger t=old+1;t<new;t++)
            {
              UIView *ms=[self viewWithTag:1000+t];
                ms.frame=CGRectMake(ms.frame.origin.x-(self->_currentControlSize.width - self->_otherControlSize.width), ms.frame.origin.y, self->_otherControlSize.width, self->_otherControlSize.height);
            }
        }
        // 右边选中到左边的时候 越过点击
        if(new-old<-1)
